...
define(function(require, exports, module) {
    ...
    /**
     * Determines whether a surface is located within the viewport.
     *
     * @private
     * @method _isOffscreen
     * @param {matrix} matrix isOffscreen matrix
     */
    Surface.prototype._isOffscreen = function _isOffscreen(matrix) {
        var surfWidth = this.options.size[0];
        var surfHeight = this.options.size[1];

        var padding = this.options.cullingPadding;
        var halfWindowWidth = window.innerWidth>>1;
        var halfWindowHeight = window.innerHeight>>1;
        var z = matrix[14];
        var y = matrix[13] + surfHeight/2;
        var x = matrix[12] + surfWidth/2;

        if(x > halfWindowWidth){
            var x = Math.abs( x - halfWindowWidth);
        } else {
            var x = Math.abs( halfWindowWidth - x );
        }

        if(y > halfWindowHeight){
            var y = Math.abs( y - halfWindowHeight);
        } else {
            var y = Math.abs( halfWindowHeight - y );
        }

        var angle = Math.atan((window.innerWidth/2 + padding)/1000);
        if( (Math.tan(angle) * (1000 - z)) + surfWidth/2 < x){
            return true;
        }
        var angle = Math.atan((window.innerHeight/2 + padding)/1000);
        if( (Math.tan(angle) * (1000 - z)) + surfHeight/2 < y){
            return true;
        }
        return false;
    }
    ...
    Surface.prototype.commit = function commit(context) {
        ...
        if(this._isOffscreen(matrix)){
            if (this._currTarget) this.cleanup(context.allocator);
            return;
        }
